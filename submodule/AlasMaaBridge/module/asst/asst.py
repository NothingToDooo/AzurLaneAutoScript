...
from .utils import JSON, InstanceOptionType
...
class Asst:
    ...
    def load(
        ...
        incremental_path: Optional[Union[pathlib.Path, str, list]] = None,
        ...
        if incremental_path:
            if isinstance(incremental_path, list):
                for i_path in incremental_path:
                    ret &= Asst.__lib.AsstLoadResource(str(i_path).encode("utf-8"))
            else:
    ...
    def connect(self, adb_path: str, address: str, config: str = "General"):
        ...
        return Asst.__lib.AsstConnect(
            self.__ptr, adb_path.encode("utf-8"), address.encode("utf-8"), config.encode("utf-8")
        )
    ...
    def append_task(self, type_name: str, params: JSON = {}) -> TaskId:
        ...
        return Asst.__lib.AsstAppendTask(
            self.__ptr, type_name.encode("utf-8"), json.dumps(params, ensure_ascii=False).encode("utf-8")
        )
