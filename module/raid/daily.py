...
if TYPE_CHECKING:
    from module.raid.raid import StandardRaidMode


class RaidStage:
    def __init__(self, name: StandardRaidMode) -> None:
        self.name = name
...
STAGES: tuple[StandardRaidMode, ...] = ("easy", "normal", "hard")
STAGE_FILTER = Filter[RaidStage](regex=re.compile(r"(\w+)"), attr=["name"])
...
class RaidDaily(RaidRun):
    def run(self, name: str = "", mode: str = "", total: int = 0) -> None:
        ...
        selected_stages = STAGE_FILTER.apply(stages)

        self.ui_ensure(page_raid)

        for stage in selected_stages:
            if isinstance(stage, str):
                message = "Raid stage filter unexpectedly returned a preset"
        ...
        configured_stages = [stage.lower().strip() for stage in self.config.RaidDaily_StageFilter.split(">")]
        if "ex" in configured_stages:
