...
class GuildLobby(GuildBase):
    def guild_lobby_get_report(self) -> Button | None:
        ...
        if len(points):
            grouped_points = Points(points).group(threshold=40)
            report_points = grouped_points + np.asarray(GUILD_REPORT_AVAILABLE.area[:2])
            # 红点位于图标右下方，向左上偏移到报告图标中心。
            area = area_offset((-51, -45, -13, 0), offset=report_points[0])
            return Button(area=area, color=(255, 255, 255), button=area, name="GUILD_REPORT")
        return None

    def _guild_lobby_collect(self, *, skip_first_screenshot: bool = True) -> None:
    ...
    def _guild_lobby_open_report(self, click_timer: Timer) -> bool:
    ...
    def _guild_lobby_handle_report_rewards(self, confirm_timer: Timer) -> bool:
    ...
    def _guild_lobby_collect_finished(self, confirm_timer: Timer) -> bool:
