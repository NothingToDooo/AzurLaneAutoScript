...
class _FoodOcr:
    def __init__(self, results: list[RecognitionResult[int]]) -> None:
        self.buttons = tuple(range(6))
        self.results = results
        self.calls: list[tuple[tuple[int, ...], bool, object]] = []

    def recognize(self, images, *, direct_ocr=False, failure_store=None):
...
class _FillOcr:
    ...
    def recognize(self, image, *, failure_store=None):
...
class _EmptyFoodFilter:
    ...
    def apply(self, food: list[Food]) -> list[Food]:
        del food
        return []
...
class _Dorm(RewardDorm):
    ...
    def _dorm_has_food(self, button) -> bool:
        del button
        return next(self._has_food)

    def image_crop(self, button, copy=True):
        del copy
        self.cropped_areas.append(button)
        return np.full((2, 2, 3), button, dtype=np.uint8)
...
def test_dorm_food_get_only_recognizes_present_slots_and_accepts_zero(monkeypatch) -> None:
...
def test_dorm_food_get_does_not_submit_empty_batch(monkeypatch) -> None:
    food_ocr = _FoodOcr([])
...
def test_dorm_food_get_retries_frame_before_fill_when_food_amount_is_invalid(monkeypatch) -> None:
...
def test_dorm_food_get_retries_frame_when_fill_is_unusable(monkeypatch, fill_result) -> None:
...
def test_dorm_feed_once_retries_after_invalid_frame(monkeypatch) -> None:
