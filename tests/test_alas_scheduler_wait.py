...
def _decision(command: str, next_run: object, *, state: ScheduleState = "waiting") -> ScheduleDecision:
...
def _wait_for_next_task(runner, decision) -> bool:
...
def test_wait_for_next_task_runs_invalid_time_through_explicit_error_state(monkeypatch) -> None:
    release_calls: list[str] = []
    monkeypatch.setattr(alas_module, "release_resources", lambda: release_calls.append("release"))
    runner = _make_runner()
